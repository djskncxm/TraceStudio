...
{
	return MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, aUtf8string, -1, NULL, 0);
}

static int sizeUtf16FromMbcs(char const *aMbcsString)
{
	return MultiByteToWideChar(CP_ACP, MB_ERR_INVALID_CHARS, aMbcsString, -1, NULL, 0);
}

static int sizeUtf8(wchar_t const *aUtf16string)
{
	return WideCharToMultiByte(CP_UTF8, WC_ERR_INVALID_CHARS, aUtf16string, -1, NULL, 0, NULL, NULL);
}

static int sizeMbcs(wchar_t const *aMbcsString)
{
	int lRes = WideCharToMultiByte(CP_ACP, 0, aMbcsString, -1, NULL, 0, NULL, NULL);
...
wchar_t *tinyfd_utf8to16(char const *aUtf8string)
{
	...
	if (lSize) {
		...
		lSize	     = MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS, aUtf8string, -1, lUtf16string, lSize);
		return lUtf16string;
	} else {
...
static void hiddenConsoleW(wchar_t const *aString, wchar_t const *aDialogTitle, int aInFront)
{
	...
	if (!CreateProcessW(NULL, (LPWSTR)aString, NULL, NULL, FALSE, CREATE_NEW_CONSOLE, NULL, NULL, &StartupInfo, &ProcessInfo)) {
		return; /* GetLastError(); */
	}
	...
	int		   aAllowMultipleSelects) /* 0 or 1 */
{
	...
	if ((!tinyfd_forceConsole || !(GetConsoleWindow() || dialogPresent())) && (!getenv("SSH_CLIENT") || getenvDISPLAY())) {
		...
		p = openFileDialogWinGui(aTitle, aDefaultPathAndOrFile, aNumOfFilterPatterns, (char const *const *)aFilterPatterns, aSingleFilterDescription, aAllowMultipleSelects);
	} else if (dialogPresent()) {
		if (aTitle && !strcmp(aTitle, "tinyfd_query")) {
			strcpy(tinyfd_response, "dialog");
			return (char *)0;
...
int tfd_isDarwin(void)
{
	...
	if (lIsDarwin < 0)
		lIsDarwin = !uname(&lUtsname) && !strcmp(lUtsname.sysname, "Darwin");
	return lIsDarwin;
}
...
int tfd_isHaiku(void)
{
	...
	struct utsname lUtsname;
	if (lIsHaiku < 0)
		lIsHaiku = !uname(&lUtsname) && !strcmp(lUtsname.sysname, "Haiku");
	return lIsHaiku;
}
...
static int detectPresence(char const *aExecutable)
{
	...
	if ((fgets(lBuff, sizeof(lBuff), lIn) != NULL) && (!strchr(lBuff, ':')) && (strncmp(lBuff, "no ", 3))) { /* present */
		pclose(lIn);
...
static int graphicMode(void)
{
	...
	if (tinyfd_forceConsole && (isTerminalRunning() || terminalName()))
		return 0;
...
int tfd_xpropPresent(void)
{
	...
	if (lXpropDetected < 0) {
		if (getenvDISPLAY() & 1)
			lXpropDetected = detectPresence("xprop"); /* bitwise & */
		else
			lXpropDetected = 0;
	}
	...
	if (!lXpropReady) { /* xwayland Debian issue reported by Kay F. Jahnke and solved with his help */
...
-e 'else' -e 'return 0' -e 'end if' ");
	...
	} else if (tfd_zenityPresent() || tfd_matedialogPresent() || tfd_shellementaryPresent() || tfd_qarmaPresent() || tfd_shantyPresent() || tfd_boxerPresent()) {
		if (tfd_zenityPresent()) {
			if (aTitle && !strcmp(aTitle, "tinyfd_query")) {
		...
		if ((tfd_zenityVersion() >= 3000) || tfd_boxerPresent()) {
			if ((tfd_zenityVersion() >= 3900) || tfd_boxerPresent())
...
to set mycolor to choose color default color {");
	...
	} else if ((tfd_zenityVersion() >= 2032) || tfd_matedialogPresent() || tfd_shellementaryPresent() || tfd_qarmaPresent() || tfd_shantyPresent() || tfd_boxerPresent()) {
		lWasZenity2032 = 1;
	...
	char const	  *aSingleFilterDescription)
{
	...
	lSavefile = tinyfd_saveFileDialog(aTitle, *aiDefaultPathAndFile, *aNumOfFilterPatterns, aFilterPatterns, aSingleFilterDescription);
	if (lSavefile)
	...
	int const	  *aAllowMultipleSelects)
{
	...
	lOpenfile = tinyfd_openFileDialog(aTitle, *aiDefaultPathAndFile, *aNumOfFilterPatterns, aFilterPatterns, aSingleFilterDescription, *aAllowMultipleSelects);
